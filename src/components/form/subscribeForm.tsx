...
export const SubscribeForm = ({ dictionary }: { dictionary: any }) => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if(validEmail && validName) {
            setSubmiting(true);
            try {
                const result = await fetch(process.env.NEXT_PUBLIC_EMAIL_PROVIDER as string, { 
                    cache: 'no-store',
                    method: 'POST',
                    body: JSON.stringify({
    ...
    const handleInputBlur = (value: string, scheme: 'email' | 'name') => {
        ...
        switch (scheme) {
            case 'name':
                ...
                !isValid && setErrorInputs(state => ({...state, [scheme]: dictionary?.form_invalid_name}));
                break;
            case 'email':
                isValid && setEmail(value);
                !isValid && setErrorInputs(state => ({...state, [scheme]: dictionary?.form_invalid_email}));
                break;
    ...
    return (
        <>
            <form 
                ...
                items-start md:w-full sm:w-full xs:w-full shadow-lg" 
                action="" 
                method="post"
            >
                <label className="font-semibold">
                    {dictionary?.form_name}
                </label>
                <input 
                ...
                />
                {errorInputs.name && <span className="text-red-500 text-sm">{errorInputs.name}</span>}
                <label className="font-semibold">
                    {dictionary?.form_email}
                </label>
                <input 
                    className="w-full py-3 px-6 border border-ab-gray rounded-lg" 
                    type="email" 
                ...
                />
                {errorInputs.email && <span className="text-red-500 text-sm">{errorInputs.email}</span>}
                <button 
